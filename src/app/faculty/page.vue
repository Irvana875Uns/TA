  ...
  return [
    ...
    },
    {
      ...
      render: (v: { id: string }) => h(Action, {
        ...
        onEdit: () => {
          router.push(`/faculty/${v.id}/update`)
        }
      }),
    },
  ]
...
const { data: faculty, isLoading, refetch } = useHttp<R<{ name: string }>>('/fakultas', {
  params
})
const { mutate: deleteAction, isPending } = useHttpMutation(computed(() => `/fakultas/${selectedId.value}`), {
  method: 'DELETE',
  queryOptions: {
    onSuccess(data,) {
...
const data = computed(() => faculty.value?.data)
</script>

<template>
  <div>
    <n-h2>
      Selamat Datang Halaman Fakultas Sistem SIGAP
    </n-h2>
  </div>
  <div>
    ...
    <div class="flex justify-between mt-5">
      ...
      <div class="flex">
        ...
        <n-pagination v-model:page="params.page" :page-count="faculty?.filter?.total_page || 1"></n-pagination>
      </div>
    </div>
  </div>
