...
const { data: faculty, isLoading, refetch } = useHttp<R<{ name: string }>>('/jenis-laporan')
const { mutate: deleteAction, isPending } = useHttpMutation(computed(() => `/jenis-laporan/${selectedId.value}`), {
  method: 'DELETE',
  queryOptions: {
    onSuccess(data,) {
...
<template>
  <div>
    <n-h2>
      Selamat Datang Halaman Jenis Pengaduan Sistem SIGAP
    </n-h2>
  </div>
  <div>
    <div class="flex flex-col md:flex-row gap-5 mb-10">
      <n-button type="primary" @click="$router.push('/report-types/create')">
        Tambah
      </n-button>
